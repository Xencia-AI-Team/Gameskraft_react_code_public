...
const ChatPage = () => {
  ...
  const onRefresh = () => {
    ...
    fetch("https://gameskraftapi.azurewebsites.net/refreshed", {
      method: "POST",
      headers: {
  ...
  const onSend = async () => {
    ...
    try {
      const response = await fetch(
        "https://gameskraftapi.azurewebsites.net/send_message",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
    ...
    <div className="w-full h-full flex flex-col bg-backgroundImg bg-opacity-50">
      ...
      <section className="bg-white flex-row flex gap-5 p-3 px-10 ">
        ...
        <div className="flex flex-col">
          ...
          <a href="mailto:grievance.officer@gameskraft.com" className="text-greyText text-sm">
